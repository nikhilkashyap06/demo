          ...
          <div className="animate-pulse">
            ...
            {[1,2,3].map((i) => (
              <div key={i} className="flex items-center py-4 border-b border-gray-200">
                <div className="h-10 w-10 bg-gray-200 rounded mr-4" />
  ...
  if (items.length === 0) {
    return (
      <div className="text-center py-12">
        ...
        <p className="mt-1 text-sm text-gray-500">Get started by creating a news item.</p>
      </div>
    );
  }
            ...
            <div className="px-4 py-4 sm:px-6">
              <div className="flex items-center justify-between">
                <div className="flex items-center">
                  {item.image_url && !String(item.image_url).startsWith('/api/') ? (
                    <img src={item.image_url} alt={item.title} className="h-10 w-10 rounded object-cover mr-4" />
                  ) : (
                  ...
                  <div>
                    <p className="text-sm font-medium text-green-600 truncate">{item.title}</p>
                    <div className="flex text-sm text-gray-500">
                      <span className="truncate">{item.publish_date || 'No publish date'}</span>
                ...
                <div className="flex items-center">
                  {item.is_published ? (
                    <span className="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-green-100 text-green-800 mr-2">Published</span>
                  ) : (
                    <span className="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-yellow-100 text-yellow-800 mr-2">Draft</span>
                  )}
                  <button
                    onClick={() => onEdit(item)}
